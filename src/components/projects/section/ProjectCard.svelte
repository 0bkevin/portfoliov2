<script lang="ts">
  ...
  export let blog: CollectionEntry<"projects">;

  const { title, description, pubYear, image, tags } = blog.data;

  const tagsToShow = tags.slice(0, 2);
</script>

<a href={`/projects/${blog.id}`}>
<article
...
>
  ...
  <div class="p-4">
    <div class="mb-3 flex items-center gap-4">
      <time class="rounded-md bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-600" dateTime={pubYear.toDateString()}>
        <i class="bx bx-calendar text-xs"></i> {formatDateToDisplay(pubYear)}
      </time>
